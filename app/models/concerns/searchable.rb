module Searchable
  ...
  included do
    ...
    scope :search_by_available, lambda {|available|
      by_type('Adoptable').by_adopted(false).by_vaccines(true).by_castrated(true).where(death_date: nil) if available == 'true'
    }
    ...
    def self.search(params)
      search_by_type(params[:type]).search_by_adopted(params[:adopted]).search_by_name(params[:name])
        ...
        .search_by_admission_date_to(params[:admission_date_to]).search_by_available(params[:available])
    end
  end
end
